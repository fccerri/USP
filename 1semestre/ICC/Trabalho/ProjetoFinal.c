...
/*
 ...
 Parâmetros de Entrada:
    Lugares - vetor de estruturas de passageiros.S
    Ocupados - número de assentos ocupados.
 */
...
/*
Na main, o programa vai ler um comando (RR, MR, CA, CR, FD, FV) e de acordo com cada um, 
o programa realiza alguma ação determinada em cada condição. Há o encerramento do código com os 
comandos FD e FV. Em alguns comandos, existe a verificação se o voo continua aberto e, assim, pode
realizar mudanças. 
*/
int main (void) {
    ...
    while(1) {
        ...
        if (!strcmp(Comando, "FV") || InfVoo.QtdAssentos == InfVoo.AssentosOcupados-InfVoo.Canceladas){
            FechamentoVoo(Lugares, &InfVoo); //Fecha o voo caso o comando seja chamado ou
            GuardaDados(Lugares, InfVoo);    //O Voo atinja a capacidade maxima de reservas
            free(Lugares);
            Lugares = NULL;
            return (0);
        
        }
        else if (!strcmp(Comando, "RR")) { //Realizar Reserva
            if(InfVoo.Status == 1){ 
            ...
            if(Lugares[i].Status == 1 || i > InfVoo.AssentosOcupados) //Trata de casos em que a reserva
                printf("Reserva não encontrada.\n");                  //não existe ou foi cancelada
            else
        ...
        else if (!strcmp(Comando, "CA")) { //Cancela Reserva
            if (InfVoo.Status == 1){ 
                FechamentoVoo(Lugares, &InfVoo);   
                scanf("%*s");      
            } 
        ...
        else if (!strcmp(Comando, "FD")) { //Fechamento Dia
...
/*
A função RealizarReserva() recebe os dados do passageiro
(Nome, Sobrenome, CPF, Data, Voo, Assento, Classe, Valor, Origem e Destino) 
que são armazenados na variável InfVoo e no vetor Lugares na ultima posição (AssentosOcupados).
Ao final é feito o incremento na variável AssentosOcupados de InfVoo.
*/
void RealizarReserva(Passageiro **Lugares, Voo *InfVoo) {      
...
int AchaReserva(Passageiro *Lugares, int Ocupados) {
    ...
    while (strcmp(Consulta, Lugares[i].CPF)!= 0 && i <= Ocupados) {  
        i++; 
    }
...
/*
A funcao ModificarReserva realiza uma modificação na reserva do passageiro, na main, já foi verificado 
se o voo está aberto. A função AchaReserva devolve o índice da  passagem que vai ser alterada. 
Primeiramente, verifica-se se essa passagem foi cancelada, se não, são lidos os novos dados e as 
informações alteradas são impressas na tela.  
*/
...
total de reservas válidas e o valor arrecadado até então.
*/
...
/*A função FechamentoVoo imprime as informacoes dos passageiros (CPF, Nome completo e Assento)
contidas no vetor Lugares e soma o valor total arrecadado pelas passagens imprimindo o valor ao final.
Sempre é feita a verificalçao se a função está ativa evitando impressoes e valores incorretos.
Ao final, o Status do InfVoo é definido para cancelado. 
*/
...
/*
A função AlocaVet aloca uma quantidade de memória correspondente ao número 'Tam' vezes
a quantidade de bytes 'Tipo' de um tipo de dado, e retorna um ponteiro void. Caso a alocação 
tenha sido malsucedida, a função imprime uma mensagem de erro e encerra o programa sinalizando falha. 
*/
